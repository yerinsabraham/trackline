...
function replay(root, raws) {
  fs.rmSync(root, { recursive: true, force: true });
  fs.mkdirSync(root, { recursive: true });
  for (const raw of raws) {
    spawnSync(path.join(bin, "trackline-hook"), ["-host", "claude", "-root", root], { input: raw, encoding: "utf8" });
  }
  ...
  return { findings, judge: review.stdout ?? "", judgeError: review.status === 0 ? "" : review.stderr };
}
...
for (const [i, name] of sessions.entries()) {
  ...
  const cases = [{ id: "control", raws }].concat(picks.map((m) => {
  ...
  for (const c of cases) {
    ...
    const r = replay(meta.root, c.raws);
